...
type CreateClientOptions = {
  useServiceRole?: boolean; // サービスロールキーを使用するかどうかのオプション
  logErrors?: boolean; // エラーログを表示するかどうかのオプション
};

export const createClient = ({ useServiceRole = false, logErrors = false }: CreateClientOptions = {}) => {
  ...
  return createServerClient<Database>(
    ...
    useServiceRole ? process.env.SUPABASE_SERVICE_ROLE_KEY! : process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, // オプションに応じて匿名キーまたはサービスロールキーを取得
    {
      cookies: {
        get(name: string) {
        ...
        set(name: string, value: string, options: CookieOptions) {
          try {
            ...
            if (logErrors) {
              console.error("Error setting cookie:", error); // エラーログの表示
            }
          }
        },
        remove(name: string, options: CookieOptions) {
          try {
            ...
            if (logErrors) {
              console.error("Error removing cookie:", error); // エラーログの表示
            }
          }
        },
      },
